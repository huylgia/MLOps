...
def squared_paired_dist(x: NDArray, y: NDArray) -> NDArray:
...
def sigma_median(dist: NDArray) -> float:
...
def rbf(x: NDArray, y: NDArray, pass_sigma: Optional[float]) -> Tuple[NDArray, float]:
...
def kernel_matrix(x: NDArray, y: NDArray) -> NDArray:
...
def mmd_2samp(kernel_matrix: NDArray, no_y_values: int, permute: bool = False) -> float:
...
def mmd_pval(x: NDArray, y: NDArray) -> Tuple[float, float]:
...
def _mmd_stattest(
    ...
    print(reference_data.shape, current_data.shape)
